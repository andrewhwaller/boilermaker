  ...
  module Settings
    class Billing < Views::Base
      ...
      def render_subscription_status
        render Components::Card.new(title: "Current Plan") do
          ...
          if subscription&.active?
            ...
            div(class: "space-y-2") do
              p(class: "text-muted") { "You don't have an active subscription." }
      ...
      def render_billing_actions
        ...
        render Components::Card.new(title: "Manage Subscription") do
      ...
      def render_invoices
        render Components::Card.new(title: "Recent Invoices") do
          div(class: "space-y-2") do
            @billable.payment_processor.charges.order(created_at: :desc).limit(5).each do |charge|
              div(class: "flex justify-between items-center py-2 border-b border-line last:border-0") do
                div do
                  span(class: "font-medium") { charge.created_at.strftime("%B %d, %Y") }
                  span(class: "text-sm text-muted ml-2") { number_to_currency(charge.amount / 100.0) }
                end
