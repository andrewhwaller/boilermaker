          ...
          def view_template
            page_with_title("Two-factor recovery codes") do
              wide_container(class: "max-w-2xl space-y-6") do
                card(content_class: "space-y-6") do
                  div(class: "space-y-3") do
                    h1(class: "text-xl font-semibold text-body") { "Keep these codes safe" }
                  ...
                  div(class: "space-y-3") do
                    h2(class: "text-xs font-semibold uppercase tracking-wide text-muted") { "Your recovery codes" }
                    div(class: "rounded-box border border-dashed border-line bg-surface p-4") do
                      ...
                      p(class: "mt-4 text-xs text-muted") do
                  ...
                  div(class: "flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between") do
                    span(class: "text-sm text-muted") { "Finished copying them? You can head back to the dashboard." }
                    link_to("OK, I'm done", root_path, class: "ui-button ui-button-secondary w-full sm:w-auto")
                  end
                end

                card(content_class: "space-y-4") do
                  h2(class: "text-lg font-semibold text-body") { "Need new recovery codes?" }
                  p(class: "text-sm text-muted") do
