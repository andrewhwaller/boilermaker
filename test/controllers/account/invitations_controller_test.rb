...
class Account::InvitationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index displays for account admin" do
    sign_in_as @admin_user
    get account_invitations_path
  ...
  test "new displays for account admin" do
    sign_in_as @admin_user
    get new_account_invitation_path
    assert_response :success
  end
  ...
  test "destroy cancels unverified invitation" do
    sign_in_as @admin_user
    invited = User.create!(email: "pending@example.com", password: "MyVerySecurePassword2024!", verified: false, account: @account)
