...
**Date:** 2025-10-08
**Version:** 1b (Second Iteration - Incorporating DHH Feedback)
**Status:** Draft
...
## Changes from Version 1a (DHH Feedback)

1. **Removed `two_factor_enabled?` alias** - Use `otp_required_for_sign_in?` directly throughout
2. **Removed `requires_two_factor_setup?` from model** - Keep config logic in controllers/views
3. **Added transaction** - Wrap disable operation for atomicity
4. **Merged DisablesController into TotpsController** - Use RESTful `#destroy` action instead
5. **Simplified enforcement** - Use `skip_before_action` instead of route allowlist
...
**Note:** Removed `two_factor_enabled?` alias and `requires_two_factor_setup?` helper per DHH feedback.

### 3. Controller Changes

#### 3.1 Update TotpsController: Add Destroy Action

**File:** `app/controllers/two_factor_authentication/profile/totps_controller.rb`

Add `destroy` action to existing controller:
...
class TwoFactorAuthentication::Profile::TotpsController < ApplicationController
  ...
  before_action :ensure_can_disable, only: %i[ destroy ]
...
Add before_action to enforce mandatory 2FA (simplified with `skip_before_action` pattern):
...
Add `destroy` route to TOTP resource:
...
Rails.application.routes.draw do
  ...
  namespace :two_factor_authentication do
    namespace :profile do
      resources :recovery_codes, only: [ :index, :create ]
      resource  :totp,           only: [ :new, :create, :update, :destroy ]  # Added :destroy
    end
  ...
  module Settings
    class Show < Views::Base
      ...
      def render_two_factor_actions
        ...
        # Turbo Frame for disable form
        if Current.user.otp_required_for_sign_in? && !Boilermaker.config.require_two_factor_authentication?
          turbo_frame_tag "disable_2fa_form"
        end
      end
    end
  end
...
#### 5.2 New View: Disable Confirmation (Turbo Frame)

**File:** `app/views/two_factor_authentication/profile/totps/destroy.rb`

Create view for disable confirmation (renders in Turbo Frame):
    ...
    module Profile
      module Totps
        class Destroy < Views::Base
          ...
          def view_template
            turbo_frame_tag "disable_2fa_form" do
              div(class: "mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg space-y-4") do
                # Warning header
                div(class: "flex items-start") do
                  ...
                  div(class: "ml-3") do
                    h3(class: "text-sm font-medium text-yellow-800") do
                      plain "Confirm disable"
                    end
                    p(class: "mt-1 text-sm text-yellow-700") do
                      plain "Enter your authentication code to confirm you want to disable two-factor authentication."
                    end
                  end
                end
...
4. Turbo Frame loads inline disable confirmation form
5. User sees warning and enters TOTP code
...
9. If invalid:
   - Re-renders form with error in Turbo Frame
...
test "disable_two_factor! deletes recovery codes" do
  user = users(:one)
  user.update!(otp_required_for_sign_in: true)
...
test "disable_two_factor! is atomic" do
  ...
  # Verify rollback - otp_required_for_sign_in should still be true
...
class TwoFactorAuthentication::Profile::TotpsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should not allow disable when 2FA not enabled" do
    ...
    delete two_factor_authentication_profile_totp_url, params: { code: "123456" }
...
class TwoFactorAuthenticationTest < ApplicationSystemTestCase
  test "user disables 2FA from settings with inline form" do
    user = users(:one)
    user.update!(otp_required_for_sign_in: true)
    sign_in_as user
    ...
    # Turbo Frame loads inline
    within "#disable_2fa_form" do
      assert_text "Confirm disable"
...
1. `app/views/two_factor_authentication/profile/totps/destroy.rb` - Disable confirmation view (Turbo Frame)
2. `test/controllers/two_factor_authentication/profile/totps_controller_test.rb` - Add destroy action tests
...
5. `app/controllers/two_factor_authentication/profile/totps_controller.rb` - Add `destroy` action and `skip_before_action`
...
9. `config/routes.rb` - Add `:destroy` to TOTP resource
...
12. `test/test_helper.rb` - Add `with_config` helper
...
Same as version 1a - deploy with flag off, notify users, then enable flag.
...
- [ ] Users can disable 2FA via DELETE to `two_factor_authentication_profile_totp_path`
...
**End of Specification Version 1b**

**Key Improvements from 1a:**
- Removed unnecessary model aliases/helpers
- Merged into existing controller (RESTful)
- Added transaction for atomicity
- Simplified enforcement with `skip_before_action`
- Cleaner, more maintainable code following Rails conventions
